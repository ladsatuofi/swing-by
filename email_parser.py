...
def date_extraction(copy_of_text):
    ...
    for date in matches:
        ...
        if date > end_date:
            end_date = date
    return (start_date.__format__('%c'), end_date.__format__('%c'))
...
def retrieve_buildings():
    #look at nitish's code, only run once, push to json file, read for every other access
        ...
        for key in link.attrs:
            if key == 'height':
                ...
                for element in link.find_all('td'):
                    count += 1
                    if count == 2 and element.string is not None and element.string.strip() != '' and type(element.find('p')) == type(element.find('hfjhefb')):
                        building_name = element.string.strip()
                    elif count == 3 and element.string is not None and element.string.strip() != '' and type(element.find('p')) == type(element.find('hfjhefb')):
